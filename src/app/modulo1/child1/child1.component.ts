...
export class Child1Component implements OnInit {
  ...
  carregarDetalhes() {
    this.apiService.get('users')  // O endpoint deve corresponder à sua API
      .subscribe({
        next: (data) => {
  ...
  transformarDetalhes(dados: any[]): any[] {
    return dados.map(item => { //map() é um método que cria um novo array com os resultados da chamada de uma função para cada elemento do array original
      return {
